...
namespace Enterprise.IIS.product.account
{
    ...
    public partial class role_edit : PageBase
    {
        ...
        {
            get { return _baseAccount ?? (_baseAccount = AccountService.FirstOrDefault(p => p.id == AccountId)); }
            set { _baseAccount = value; }
        }

        protected int AccountId
        {
            get { return int.Parse(Request["accountid"]); }
        }
        ...
        protected void btnSava_Click(object sender, EventArgs e)
        {
            ...
            switch (Actions)
            {
                ...
                case WebAction.Deal:
                    ...
                    if (BaseAccount != null)
                    {
                        var role = RoleService.Where(p => p.account_id == BaseAccount.id).FirstOrDefault();
                        if (role == null)
                        {
                            BaseRole = new base_role
                            {
                            ...
                            if (RoleService.Add(BaseRole))
                            {
                                BaseAccount.account_role_indicator = 2;
                                BaseAccount.account_role_id = BaseRole.id.ToString(CultureInfo.InvariantCulture);
                                BaseAccount.account_role_name = "个人授权";
                        ...
                        {
                            role.role_menu_code = sbRoleMenuCode.ToString();
                            role.role_action = sbRoleAction.ToString();
                            role.role_menu_code_p = sbRoleMenuCodeP.ToString();
                            role.role_name = "个人授权";
                            role.role_desc = "个人授权";
                            role.account_id = BaseAccount.id;

                            if (RoleService.SaveChanges() >= 0)
                            {
                                BaseAccount.account_role_indicator = 2;
                                BaseAccount.account_role_id = role.id.ToString(CultureInfo.InvariantCulture);
                                BaseAccount.account_role_name = "个人授权";
                                AccountService.SaveChanges();
        ...
        {
            Response.Redirect(Request["ReturnUrl"] ?? "account_list.aspx");
        }
    }
}