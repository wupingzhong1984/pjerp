...
function onReady() {
    ...
    var btnShowHideHeader = Ext.getCmp(IDS.btnShowHideHeader);
    var regionTop = Ext.getCmp(IDS.regionTop);
    var regionPanel1 = Ext.getCmp(IDS.RegionPanel1);
    ...
    btnShowHideHeader.on('click', function () {
        if (regionTop.getInnerHeight() > 0) {
            ...
            btnShowHideHeader.setText('显示标题栏');
           btnShowHideHeader.setIcon('res.axd?icon=SectionCollapsed');
        } else {
            regionTop.setHeight(100);
            btnShowHideHeader.setText('隐藏标题栏');
