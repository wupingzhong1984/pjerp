<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Index.aspx.cs" Inherits="Enterprise.IIS.business.AllotTrans.Index" %>
...
<body>
                ...
                <f:Region ID="Region1" Title="调拨出库单" Position="Center"
                    ...
                    <Toolbars>
                        <f:Toolbar ID="Toolbar1" runat="server">
                            <Items>
                                <f:Label runat="server" ID="headerText" Text="调拨出库单"/>
                                    ...
                                    <Items>
                                        <f:Grid ID="Grid1" PageSize="200" ShowBorder="false" ShowHeader="false" AllowPaging="true"
                                            ...
                                            <Columns>
                                                ...
                                                <f:BoundField MinWidth="120px" DataField="FDistributionPoint" HeaderText="调拨站点" SortField="FDistributionPoint" />
                                                <f:BoundField MinWidth="120px" DataField="FPoint" HeaderText="接收站点" SortField="FPoint" />
    ...
    <script type="text/javascript">
        ...
        function openDetailsUI(keyid) {
            var url = 'business/Allot/Details.aspx?action=6&keyid=' + keyid;
            parent.addExampleTab.apply(null, ['add_tab_' + keyid, basePath + url, '【调拨出库单' + keyid + '详情】', basePath + 'icon/page_find.png', true]);
        };
        function openAddUI() {
            var url = 'business/Allot/Edit.aspx?action=1';
            parent.addExampleTab.apply(null, ['add_tab_00001', basePath + url, '添加调拨出库单', basePath + 'icon/page_add.png', true]);
        };
        function openEditUI(keyid) {
            var url = 'business/Allot/Edit.aspx?KeyId=' + keyid + '&action=2';
            parent.addExampleTab.apply(null, ['add_tab_00001', basePath + url, '【编辑调拨出库单' + keyid + '】', basePath + 'icon/page_edit.png', true]);
        };
        ...
        function LodopPrinter(keyid) {
            ...
            var strHtml = "";
            $.post("../../common/AjaxPrinter.ashx?oper=ajaxPrintAllotTrans",
				  { "keyid": keyid },
              function (result) {
                  if (result != "") {
