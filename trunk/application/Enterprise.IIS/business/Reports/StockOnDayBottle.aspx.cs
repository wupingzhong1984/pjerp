...
{
    public partial class StockOnDayBottle : PageBase
    {
        /// <summary>
        ///     排序字段
        /// </summary>
        ...
        private void BindDataGrid()
        {
            ...
            var data = SqlService.ExecuteProcedureCommand("rpt_dayInventory", parms).Tables[0];
                ...
                var summary = new JObject
                    {
                        ...
                        {"FInitGQty", data.Compute("sum(FInitGQty)","true").ToString()},
                        {"FInitBQty", data.Compute("sum(FInitBQty)","true").ToString()},

                        {"FInTotal", data.Compute("sum(FInTotal)","true").ToString()},
                        {"FGInQty", data.Compute("sum(FGInQty)","true").ToString()},
                        {"FBInQty", data.Compute("sum(FBInQty)","true").ToString()},

                        {"FOutTotal", data.Compute("sum(FOutTotal)","true").ToString()},
                        {"FGOutQty", data.Compute("sum(FGOutQty)","true").ToString()},
                        {"FBOutQty", data.Compute("sum(FBOutQty)","true").ToString()},

                        {"FEndTotal", data.Compute("sum(FEndTotal)","true").ToString()},
                        {"FEndGQty", data.Compute("sum(FEndGQty)","true").ToString()},
                        {"FEndBQty", data.Compute("sum(FEndBQty)","true").ToString()},
