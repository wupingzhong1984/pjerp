...
{
    public partial class DynamicGasBlock : PageBase
    { /// <summary>
      ///     排序字段
      /// </summary>
        ...
        private void BindDataGrid()
        {
            ...
            parms.Add("@FDate", Convert.ToDateTime(dpEDate.SelectedDate).ToString("yyyy-MM-dd"));
            parms.Add("@FEndDate", Convert.ToDateTime(dpFDate.SelectedDate).ToString("yyyy-MM-dd"));
            parms.Add("@FPoint", ddlFDistributionPoint.SelectedValue);
            parms.Add("@FBottle", txtFName.Text.Trim());

            var data = SqlService.ExecuteProcedureCommand("rpt_DynamicGasTimeArea", parms).Tables[0];
