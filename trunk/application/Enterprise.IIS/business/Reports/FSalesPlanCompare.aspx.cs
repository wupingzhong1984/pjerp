...
{
    public partial class FSalesPlanCompare : PageBase
    {
        /// <summary>
        ///     排序字段
        /// </summary>
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", string.Format(@"attachment; filename=销售订单与发货对比分析{0}.xls", SequenceGuid.GetGuidReplace()));
        ...
        private void BindDataGrid()
        {
            ...
            var list = SqlService.ExecuteProcedureCommand("rpt_FSalesPlanCompare", parms).Tables[0];
            ...
            if (list != null && list.Rows.Count > 0)
            {
                ...
                var summary = new JObject
                    {
                        {"FName", "合计"},
                        {"FPlanQty", list.Compute("sum(FPlanQty)","true").ToString()},
                        {"FQty", list.Compute("sum(FQty)","true").ToString()},
                        {"FDiffQty", list.Compute("sum(FDiffQty)","true").ToString()},
                    };
