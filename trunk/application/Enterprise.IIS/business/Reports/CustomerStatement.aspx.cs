...
{
    public partial class CustomerStatement : PageBase
    {
        /// <summary>
        ///     排序字段
        /// </summary>
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", string.Format(@"attachment; filename=销货对账单{0}[{1}-{2}].xls", tbxFCustomer.Text.Trim(),
                Convert.ToDateTime(dpkFDateBegin.SelectedDate).ToString("yyyy-MM-dd"),
                Convert.ToDateTime(dpkFDateEnd.SelectedDate).ToString("yyyy-MM-dd")
                ));
        ...
        private void BindDataGrid()
        {
            ...
            parms.Add("@FItemCode", txtItemCode.Text);
            parms.Add("@KeyId", txtFKeyId.Text);


            var list = SqlService.ExecuteProcedureCommand("proc_CustomerStatement", parms).Tables[0];
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                PageContext.RegisterStartupScript(string.Format("LodopPrinter('{0}','{1}','{2}',{3});", //
                    txtFCode.Text,//
                    Convert.ToDateTime(dpkFDateBegin.SelectedDate).ToString("yyyy-MM-dd"),
                    Convert.ToDateTime(dpkFDateEnd.SelectedDate).ToString("yyyy-MM-dd"),
