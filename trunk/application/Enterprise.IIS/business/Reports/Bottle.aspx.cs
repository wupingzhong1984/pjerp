...
{
    public partial class Bottle : PageBase
    {
        /// <summary>
        ///     排序字段
        /// </summary>
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", string.Format(@"attachment; filename=钢瓶库存盘点表{0}.xls", SequenceGuid.GetGuidReplace()));
        ...
        private void BindDataGrid()
        {
            ...
            parms.Add("@BottleFCode", txtFBottleCode.Text);
            if (trDept.SelectedNodeID != null && !string.IsNullOrEmpty(trDept.SelectedNodeID) &&
                trDept.SelectedNodeID != "2001")
            {
                parms.Add("@FCateId", trDept.SelectedNodeID);
            }
            else
            {
                parms.Add("@FCateId", "-1");
            ...
            var data = SqlService.ExecuteProcedureCommand("rpt_Bottle", parms).Tables[0];
            ...
            if (data != null && data.Rows.Count > 0)
            {
                ...
                {
                    {"FName", "合计"},
                    {"FInitQty", data.Compute("sum(FInitQty)", "true").ToString()},
                    {"FPurchaseQty", data.Compute("sum(FPurchaseQty)", "true").ToString()},
                    {"FSalaryQty", data.Compute("sum(FSalaryQty)", "true").ToString()},
