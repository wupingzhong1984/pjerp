...
namespace Enterprise.IIS.business.BottleDetection
{
    public partial class Edit : PageBase
    {
        ...
        ///     数据服务
        /// </summary>
        private BottleDetectionDetailsService _bottleDetectionDetailsService;

        /// <summary>
        ///     数据服务
        /// </summary>
        protected BottleDetectionDetailsService BottleDetectionDetailsService
        {
            get
            {
                return _bottleDetectionDetailsService ?? //
                    (_bottleDetectionDetailsService = new BottleDetectionDetailsService());
            }
            set { _bottleDetectionDetailsService = value; }
        }
        ...
        ///     
        /// </summary>
        private LHBottleDetection _stockOut;

        /// <summary>
        ///     
        /// </summary>
        protected LHBottleDetection StockOut
        {
            get
            {
                return _stockOut ?? (_stockOut = BottleDetectionService.FirstOrDefault(p => p.KeyId == KeyId //
                    && p.FCompanyId == CurrentUser.AccountComId));
            }
            set { _stockOut = value; }
        }
                ...
                if (GetRequestEventArgument().Contains("reloadGrid:"))
                {
                    ...
                    if (data != null && data.Tables.Count > 0 && data.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < table.Rows.Count; i++)
                        {
                            ...
                            BottleDetectionDetailsService.AddEntity(details);
                        ...
                        BottleDetectionDetailsService.SaveChanges();
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            if (e.CommandName == "Delete" || e.CommandName == "Add")
            {
                ...
                BottleDetectionDetailsService.Delete(p => p.FId == datakey && p.FCompanyId == CurrentUser.AccountComId);
        ...
        private void ModifiedGrid()
        {
            ...
            foreach (var rowKey in dictModified.Keys)
            {
                ...
                var details = BottleDetectionDetailsService.Where(p => p.FId == datakey).FirstOrDefault();
                //写入原始，通过存储过程完成明细复制
