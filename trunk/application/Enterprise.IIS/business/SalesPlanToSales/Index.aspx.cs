...
namespace Enterprise.IIS.business.SalesPlanToSales
{
    public partial class Index : PageBase
    {
            ...
            if (item != null)
            {
                ...
                if (item.FAuditFlag == 1)
                {
                    ...
                    if (item.FStatus == 50)
                    {
                        ...
                        {
                            if (!e.Values[i].ToString().Contains("#@TPL@"))
                                e.Values[i] = string.Format("<span class=\"{0}\">{1}</span>", "colorblueviolet", e.Values[i].ToString().Contains("#@TPL@") ? "已作废" : e.Values[i]);
                        }
                    }
                    else
                    {
        ...
        protected void btnSalesAuto_Click(object sender, EventArgs e)
        {
            IEnumerable<string> selectIds = GetSelectIds();
            ...
            try
            {
                ...
                foreach (string ids in selectIds)
                {
                    LHStockOut stockOut = new StockOutService().Where(p => p.FLogisticsNumber == ids && p.FFlag != 0).FirstOrDefault();
                    if (stockOut == null)
                    {

                        parms.Clear();
                        parms.Add("@OCKeyId", ids);
                        parms.Add("@FData", DateTime.Now.ToString("yyyy-MM-dd"));
                        parms.Add("@FCompanyId", CurrentUser.AccountComId);
                        parms.Add("@CreateBy", CurrentUser.AccountName);

                        SqlService.ExecuteProcedureCommand("proc_SalesPlanToSales", parms);

                        try
                        {
                            tmsModel t = new tmsModel();
                            LHStockOut hStockOut = new StockOutService().Where(p => p.FLogisticsNumber == ids).FirstOrDefault();
                            List<LHStockOut> lHs = new List<LHStockOut>
                        {
                            hStockOut
                        };
                            t.stockOutList = lHs;
                            t.stockOutDetailsList = new StockOutDetailsService().Where(p => p.KeyId == ids).ToList();
                            t.passCardList = new List<LHPassCard>();
                            t.passCardList = StockOutService.Where(p => p.KeyId == ids).ToList();
                            t.passCardDetailsList = new List<LHPassCardDetails>();
                            t.passCardDetailsList = new PassCardDetailsService().Where(p => p.KeyId == ids).ToList();
        ...
        private void BindDataGrid()
        {
            ...
            int output;
            Expression<Func<LHPassCard, bool>> predicate = BuildPredicate(out  orderingSelector);
