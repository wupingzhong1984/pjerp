    ...
    public partial class Edit : PageBase
    {
                ...
                if (GetRequestEventArgument().Contains("reloadGrid:"))
                {
                    ...
                    if (data != null && data.Tables.Count > 0 && data.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < table.Rows.Count; i++)
                        {
                            ...
                            var itemCode = table.Rows[i]["FItemCode"].ToString();

                            decimal price = GasHelper.GeCustomerPrice(txtFCode.Text.Trim(), //
                                itemCode, CurrentUser.AccountComId);

                            var info = new CustomerPriceService().FirstOrDefault(p => p.FCode == txtFCode.Text && p.FItemCode == itemCode);
        ...
        protected void tbxFCustomer_OnTextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbxFCustomer.Text.Trim()))
            {
                ...
                if (custmoer != null)
                {
                    ...
                    {
                        var id = Convert.ToInt32(ddlFOrgName.SelectedValue);
        ...
        private bool SubmintEdit()
        {
            if (StockOut != null)
            {
                ...
                var cusinfo = CustomerService.FirstOrDefault(p => p.FCode == txtFCode.Text.Trim());
