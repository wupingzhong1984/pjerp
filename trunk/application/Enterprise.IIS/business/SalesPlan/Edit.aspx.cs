    ...
    public partial class Edit : PageBase
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    case WebAction.Add:
                        while (i <= int.Parse(string.IsNullOrEmpty(pubNum.Text) ? "1" : pubNum.Text))
                        {
                            isSucceed = SubmintAdd();
                            i++;
                            if (i<= int.Parse(string.IsNullOrEmpty(pubNum.Text) ? "1" : pubNum.Text))
                            {
                                LHPassCardDetails newdetails = StockOutDetailsService.Where(p => p.KeyId == txtKeyId.Text).FirstOrDefault();
                                txtKeyId.Text = SequenceService.CreateSequence("TM", CurrentUser.AccountComId);
                                newdetails.KeyId = txtKeyId.Text;
                                ...
                                StockOutDetailsService.Add(newdetails);
                            }
