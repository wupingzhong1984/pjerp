...
{
    public partial class PreIndex : PageBase
    {
        /// <summary>
        ///     排序字段
        /// </summary>
        ...
        ///     
        /// </summary>
        private StockOutService _passcardOutService;

        /// <summary>
        ///     
        /// </summary>
        protected StockOutService PasscardOutService
        {
            get { return _passcardOutService ?? (_passcardOutService = new StockOutService()); }
            set { _passcardOutService = value; }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                dateBegin.SelectedDate = DateTime.Now.AddDays(1);

                dateEnd.SelectedDate = DateTime.Now.AddDays(1);

                //btnAdd.OnClientClick = Window1.GetShowReference(string.Format("./Edit.aspx?action=1"), "销售订单");
        ...
        {
            LHPassCard item = e.DataItem as LHPassCard;
        ...
        protected void btnBatchDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                StockOutService.Update(p => p.FCompanyId == CurrentUser.AccountComId && selectIds.Contains(p.KeyId), p => new LHPassCard
                {
                    FFlag = 0, //
        ...
        protected void btnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    //验证当前单据是否已发货，如果已发货，不允许内勤再次变更该单据。
                    LHStockOut info = new StockOutService().FirstOrDefault(p => p.FLogisticsNumber == sid//
                    && p.FCompanyId == CurrentUser.AccountComId);

                    if (info != null)
                    {
                        Alert.Show("该订单已发货，不允许再次修改原始订单。", MessageBoxIcon.Information);
                        return;
                    }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btnSendPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (Grid1.SelectedRowIndexArray.Length == 0)
                ...
                else
                {
                    ...
                    PageContext.RegisterStartupScript(string.Format("LodopSendPrinter('{0}');", sid));
                }
            }
            catch (Exception)
            {
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", string.Format(@"attachment; filename=销售订单{0}.xls", SequenceGuid.GetGuidReplace()));
        ...
        private void BindDataGrid()
        {
            ...
            Expression<Func<LHPassCard, bool>> predicate = BuildPredicate(out dynamic orderingSelector);

            //取数据源
            IQueryable<LHPassCard> list = StockOutService.Where(predicate, Grid1.PageSize, Grid1.PageIndex + 1,
                orderingSelector, EnumHelper.ParseEnumByString<OrderingOrders>(SortDirection), out int output);
        ...
        /// <returns></returns>
        private Expression<Func<LHPassCard, bool>> BuildPredicate(out dynamic orderingSelector)
        {
            // 查询条件表达式
            Expression expr = Expression.Constant(true);

            ParameterExpression parameter = Expression.Parameter(typeof(LHPassCard));
            ...
            expr = Expression.And(expr,
               Expression.Equal(Expression.Property(parameter, "FStatus"), Expression.Constant(9, typeof(int?))));


            expr = Expression.And(expr,
                    Expression.Call(Expression.Property(parameter, "CreateBy"), methodInfo,
                        // ReSharper disable once PossiblyMistakenUseOfParamsMethod
                        Expression.Constant(CurrentUser.AccountName)));
            ...
            base_employee emp = new EmployeeService().FirstOrDefault(p => p.number == CurrentUser.AccountJobNumber);
            if (emp != null)
            {
            ...
            Expression<Func<LHPassCard, bool>> predicate = Expression.Lambda<Func<LHPassCard, bool>>(expr, parameter);
        ...
        protected void btnFormal_Click(object sender, EventArgs e)
        {
            try
            {
                IEnumerable<string> ids = GetSelectIds();
                StockOutService.Update(p => p.FCompanyId == CurrentUser.AccountComId && ids.Contains(p.KeyId), p => new LHPassCard
                {
                    FStatus = Convert.ToInt32(GasEnumBillStauts.Add)
                });
                Alert.Show("转单成功！", MessageBoxIcon.Information);
                BindDataGrid();
            }
            catch (Exception)
            {
