...
{
    public partial class Audit : PageBase
    {
        #region  Service
        /// <summary>
        ///     AppendToEnd
        ...
        ///     数据服务
        /// </summary>
        private SupplierService _customerService;
        ...
        {
            get { return _customerService ?? (_customerService = new SupplierService()); }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btnBillFlow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                PageContext.RegisterStartupScript(
                    Window2.GetShowReference(string.Format("../../Common/WinBillFlow.aspx?KeyId={0}&action=2&Bill=45",
                        KeyId), "审批过程详情", 480, 400));
            }
            catch (Exception)
            {
                isSucceed = false;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                PageContext.RegisterStartupScript(
                    Window2.GetShowReference(string.Format("../../Common/WinAudit.aspx?KeyId={0}&action=2&Bill=45",
                        KeyId), "审核", 480, 400));
            }
            catch (Exception)
            {
        ...
        {
            var source = SqlService.Where(string.Format(@"SELECT * FROM dbo.vm_PassCardDetails WHERE keyId='{0}' and FCompanyId={1}", txtKeyId.Text, CurrentUser.AccountComId));
        ...
        protected void tbxFCustomer_OnTextChanged(object sender, EventArgs e)
        {
            ...
            {
                var custmoer = SupplierService.Where(p => p.FName == tbxFCustomer.Text.Trim() //
