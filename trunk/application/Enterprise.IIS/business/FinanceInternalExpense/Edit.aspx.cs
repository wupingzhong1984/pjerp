...
namespace Enterprise.IIS.business.FinanceInternalExpense
{
    public partial class Edit : PageBase
    {
        ...
        ///     客户档案
        /// </summary>
        private base_employee _employee;

        /// <summary>
        ///     客户档案
        /// </summary>
        protected base_employee Employee
        {
            get { return _employee ?? (_employee = EmployeeService.FirstOrDefault(p => p.job_number == txtFCode.Text.Trim() && p.FCompanyId == CurrentUser.AccountComId)); }
            set { _employee = value; }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (GetRequestEventArgument().Contains("reloadGrid:"))
                {
                    ...
                    var data = SqlService.Where(string.Format("SELECT * FROM dbo.vm_Bank a WHERE a.FCode IN ({0}) and FCompanyId={1} and FDate='{2}'", value, CurrentUser.AccountComId, txtFDate.SelectedDate.Value.ToString("yyyy-MM")));
                ...
                if (GetRequestEventArgument().Contains("reloadTree:"))
                {
                    ...
                    var data = SqlService.Where(string.Format("SELECT * FROM dbo.vm_Subject a WHERE a.FCode IN ({0}) and FCompanyId={1} and FDate='{2}'", value, CurrentUser.AccountComId, txtFDate.SelectedDate.Value.ToString("yyyy-MM")));
...
//----------------------------------------------------------------------------
        ...
        protected void tbxFCustomer_OnTextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbxFCustomer.Text.Trim()))
            {
                ...
                var custmoer = EmployeeService.Where(p => p.name == tbxFCustomer.Text.Trim() && p.FCompanyId == CurrentUser.AccountComId).FirstOrDefault();
                if (custmoer != null)
                {
                    txtFCode.Text = custmoer.job_number;
                }
            }
        }
            ...
            switch (Actions)
            {
                case WebAction.Add:
                    ...
                    var temp = new LHFKOrder
                    {
                        ...
                        //现金费用单
                        FType = Convert.ToInt32(GasEnumBill.InternalExpense),
