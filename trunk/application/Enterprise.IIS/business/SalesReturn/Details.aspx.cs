...
namespace Enterprise.IIS.business.SalesReturn
{
  ...
  LEFT JOIN dbo.base_orgnization b ON a.FDeptId=b.id WHERE a.KeyId='{0}' and a.FCompanyId={1}", txtKeyId.Text, CurrentUser.AccountComId));
            //绑定数据源
            ...
            switch (Actions)
            {
                case WebAction.Details:
                    if (StockIn != null)
                    {
                        ...
                        txtFAddress.Text = StockIn.FAddress;
                        txtFFreight.Text = StockIn.FFreight.ToString();
                        txtFLinkman.Text = StockIn.FLinkman;
                        txtFMemo.Text = StockIn.FMemo;
                        txtFPhone.Text = StockIn.FPhone;
                        ddlFDriver.Text = StockIn.FDriver;
                        ddlFShipper.Text = StockIn.FShipper;
                        ddlFSupercargo.Text = StockIn.FSupercargo;
                        ddlFVehicleNum.Text = StockIn.FVehicleNum;
                        txtFDate.Text = Convert.ToDateTime(StockIn.FDate).ToString("yyyy-MM-dd");
