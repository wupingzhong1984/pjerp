    ...
    public partial class Edit : PageBase
    {
        ...
        private bool SubmintAdd()
        {
            ...
            if (stock != null)
            {
                ...
                stock.FAmt = 0;////Convert.ToDecimal(txtFAmt.Text.Trim());
                ...
                {
                    //单据号问题
                    string newKeyId = SequenceService.CreateSequence(Convert.ToDateTime(txtFDate.SelectedDate), "KC", CurrentUser.AccountComId);
                    var orderParms = new Dictionary<string, object>();
                    orderParms.Clear();
                    orderParms.Add("@oldKeyId", txtKeyId.Text);
                    orderParms.Add("@newKeyId", newKeyId);
                    orderParms.Add("@Bill", "16");
                    ...
                    var billStatus = new LHBillStatus
                    {
                        ...
                        FMemo = String.Format("单据号{0},{1}新增空瓶出库单据。", newKeyId, CurrentUser.AccountName)
                    };
