...
namespace Enterprise.IIS.business.BottleTo
{
  ...
  LEFT JOIN dbo.base_orgnization b ON a.FDeptId=b.id WHERE KeyId='{0}' and a.FCompanyId={1}", lblKeyId.Text,CurrentUser.AccountComId));
        ...
        {
            lblCreateBy.Text = CurrentUser.AccountName;
        ...
        private void LoadData()
        {
            switch (Actions)
            {
                case WebAction.Details:
                    ...
                    if (StockOut != null)
                    {
                        lblCustomer.Text = StockOut.FName;
                        lblFAddress.Text = StockOut.FAddress;
                        lblFFreight.Text = StockOut.FFreight.ToString();
                        lblFLinkman.Text = StockOut.FLinkman;
                        lblFMemo.Text = StockOut.FMemo;
                        lblFPhone.Text = StockOut.FPhone;
                        lblFDriver.Text = StockOut.FDriver;
                        lblFShipper.Text = StockOut.FShipper;
                        lblFSupercargo.Text = StockOut.FSupercargo;
                        lblFVehicleNum.Text = StockOut.FVehicleNum;
                        lblFDate.Text = Convert.ToDateTime(StockOut.FDate).ToString("yyyy-MM-dd");
