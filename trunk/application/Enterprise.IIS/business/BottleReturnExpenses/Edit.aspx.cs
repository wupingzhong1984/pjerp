...
namespace Enterprise.IIS.business.BottleReturnExpenses
{
    public partial class Edit : PageBase
    {
                ...
                for (int i = 0; i < keys.Count(); i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(key))
                    {
                        if (details != null)
                        {
                            ...
                            if (key.Equals("FOutTemperature"))//外数
                            {
                                details.FOutTemperature = Convert.ToDecimal(value);
                            }
                            if (key.Equals("FInTemperature"))//外费
                            {
                                details.FInTemperature = Convert.ToDecimal(value);
                            }
                            if (key.Equals("FInPressure"))//外折
                            {
                                details.FInPressure = Convert.ToDecimal(value);
                            }
                            if (key.Equals("FOutPressure"))//外折
                            {
                                details.FOutPressure = Convert.ToDecimal(value);
                            }
        ...
        private bool SubmintAdd()
        {
            ...
            if (stock != null)
            {
                ...
                {
                    // 单据类型
                    int ftype = Convert.ToInt32(GasEnumBill.BottleReturnExpenses);

                    //单据号问题
                    string newKeyId = SequenceService.CreateSequence(Convert.ToDateTime(txtFDate.SelectedDate), "FL", CurrentUser.AccountComId);
                    var orderParms = new Dictionary<string, object>();
                    orderParms.Clear();
                    orderParms.Add("@oldKeyId", txtKeyId.Text);
                    orderParms.Add("@newKeyId", newKeyId);
                    orderParms.Add("@Bill", ftype);
            ...
            switch (Actions)
            {
                case WebAction.Add:
                    ...
                    var temp = new LHStockOut
                    {
                        ...
                        //发货单
                        FType = Convert.ToInt32(GasEnumBill.BottleReturnExpenses),
