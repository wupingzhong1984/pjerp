...
{
    public partial class LiquidSalesEdit : PageBase
    {
        #region  Service
        /// <summary>
        ///     AppendToEnd
        ...
        private bool SubmintAdd()
        {
            ...
            if (stock != null)
            {
                ...
                {
                    //单据号问题
                    string newKeyId = SequenceService.CreateSequence(Convert.ToDateTime(txtFDate.SelectedDate), "LX", CurrentUser.AccountComId);
                    var orderParms = new Dictionary<string, object>();
                    orderParms.Clear();
                    orderParms.Add("@oldKeyId", txtKeyId.Text);
                    orderParms.Add("@newKeyId", newKeyId);
                    orderParms.Add("@Bill", "28");
