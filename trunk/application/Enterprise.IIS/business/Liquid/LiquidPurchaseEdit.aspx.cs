...
{
    public partial class LiquidPurchaseEdit : PageBase
    {
        /// <summary>
        ///     AppendToEnd
        /// </summary>
                ...
                if (firstOrDefault != null && !string.IsNullOrEmpty(firstOrDefault.ToString()))
                {
                    ...
                    if (table != null && table.Rows.Count > 0)
                    {
                        ...
                        var details = new LHStockInDetails
                        {
                            ...
                            FCateId = 2000//Convert.ToInt32(table.Rows[0]["FId"].ToString())
                        };
        ...
        private bool SubmintAdd()
        {
            ...
            if (stock != null)
            {
                ...
                {
                    //单据号问题
                    string newKeyId = SequenceService.CreateSequence(Convert.ToDateTime(txtFDate.SelectedDate), "LC", CurrentUser.AccountComId);
                    var orderParms = new Dictionary<string, object>();
                    orderParms.Clear();
                    orderParms.Add("@oldKeyId", txtKeyId.Text);
                    orderParms.Add("@newKeyId", newKeyId);
                    orderParms.Add("@Bill", "27");
            ...
            switch (Actions)
            {
                case WebAction.Add:
                    ...
                    var temp = new LHStockIn
                    {
                        ...
                        FType = Convert.ToInt32(GasEnumBill.LiquidPurchase),
