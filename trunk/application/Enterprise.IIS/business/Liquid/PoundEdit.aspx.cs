    ...
    public partial class PoundEdit : PageBase
    {
        ...
        protected void tbxFName1_OnTextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbxFName1.Text.Trim()))
            {
                ...
                var custmoer = CustomerService.Where(p => p.FName == tbxFName1.Text.Trim() //
                    && p.FCompanyId == CurrentUser.AccountComId).FirstOrDefault();
                if (custmoer != null)
                {
        ...
        private void LoadData()
        {
            switch (Actions)
            {
                ...
                case WebAction.Edit:
                    if (LiquidPlan != null)
                    {
                        ddlFSalesman.SelectedValue = LiquidPlan.CreateBy;

                        FTime1.Text = LiquidPlan.FTime1;
                        FTime2.Text = LiquidPlan.FTime2;
                        FTime2.Text = LiquidPlan.FTime3;

                        FDate4.SelectedDate= LiquidPlan.FDate4;
