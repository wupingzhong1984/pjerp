...
namespace Enterprise.IIS.business.TubeSales
{
    public partial class Edit : PageBase
    {
                ...
                if (isSucceed)
                {
                        ...
                        List<LHStockOut> lHs = new List<LHStockOut>
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(tbxFLogisticsNumber.Text))
                        {
                            ...
                            t.passCardDetailsList = new PassCardDetailsService().Where(p => p.KeyId == tbxFLogisticsNumber.Text).ToList();
                        }


                        IQueryable<LHStockIn> lHStockIns = new StockInService().Where(p => p.FLogisticsNumber == txtKeyId.Text);
                        if (lHStockIns != null && lHStockIns.Count() > 0)
                        {
                            t.stockInList = lHStockIns.ToList();
        ...
        protected void Grid1_RowDoubleClick(object sender, GridRowClickEventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(txtFCode.Text.Trim()))
                {
                    Alert.Show("请选择机构信息，再输入产品信息。", MessageBoxIcon.Error);
                    return;
                }
                ...
                PageContext.RegisterStartupScript(
                    Window1.GetShowReference(string.Format("./D.aspx?FId={0}&KeyId={1}&action=2&FCode={2}",
                        sid, keyId, txtFCode.Text.Trim()), "编辑收发数量"));
            }
            catch (Exception ex)
            {
        ...
        protected void tbxFLogisticsNumber_OnTextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbxFLogisticsNumber.Text.Trim()))
            {
                ...
                if (passCard != null)
                {
                    GasHelper.DropDownListOrgDataBind(ddlFOrgName, passCard.FCode);

                    ddlFOrgName.SelectedValue = passCard.FOrgId.ToString();

                    //配送方式
                    ddlDeliveryMethod.SelectedValue = passCard.FDeliveryMethod;
                    ddlFDistributionPoint.SelectedValue = passCard.FDistributionPoint;
                    ddlFSalesman.SelectedValue = passCard.CreateBy;

                    txtFCode.Text = passCard.FCode;
                    tbxFCustomer.Text = passCard.FName;

                    LHDispatchCenter dis = new DispatchCenterService().Where(p => p.KeyId == passCard.FDispatchNum).FirstOrDefault();
                    if (dis != null)
                    {
        ...
        private bool SubmintEdit()
        {
            if (StockOut != null)
            {
                ...
                //---------------------------------------------------------
                //收发类型
                StockOut.FT6Date = txtFDate.SelectedDate;
                ...
                Dictionary<string, object> bottles = new Dictionary<string, object>();
                bottles.Clear();
        ...
        private bool SubmintAdd()
        {
            ...
            if (stock != null)
            {
                ...
                {
                    //单据号问题
                    string newKeyId = SequenceService.CreateSequence(Convert.ToDateTime(txtFDate.SelectedDate), "TS", CurrentUser.AccountComId);
