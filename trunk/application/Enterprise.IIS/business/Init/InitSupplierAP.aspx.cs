...
{
    public partial class InitSupplierAP : PageBase
    {
        /// <summary>
        ///     数据服务
        /// </summary>
        private InitSupplierAPService _initSupplierAPService;

        /// <summary>
        ///     数据服务
        /// </summary>
        protected InitSupplierAPService InitSupplierAPService
        {
            get { return _initSupplierAPService ?? (_initSupplierAPService = new InitSupplierAPService()); }
            set { _initSupplierAPService = value; }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Label1.Text = string.Format(@"引入供应商应付账款表");
            }
        }
            ...
            try
            {
                using (var ts = new TransactionScope())
                {
                    ...
                    for (int i = 0; i < data.Rows.Count; i++)
                    {
                        ...
                        var item = new LHInitSupplierAP
                        {
                            FCode = data.Rows[i]["供应商编码"].ToString(),
                            FDate = data.Rows[i]["供应商名称"].ToString(),
                            FAmt = Convert.ToDecimal(data.Rows[i]["应付账款"])
                        };

                        InitSupplierAPService.AddEntity(item);
                    }

                    //写入数据库
                    InitSupplierAPService.SaveChanges();
