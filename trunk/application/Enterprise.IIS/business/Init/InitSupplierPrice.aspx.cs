...
namespace Enterprise.IIS.business.Init
{
    ...
    ///     初始供应商对应产品采购价
    /// </summary>
    public partial class InitSupplierPrice : PageBase
    {
        /// <summary>
        ///     数据服务
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Label1.Text = string.Format(@"引入供应商对应产品采购表");
            }
        }
            ...
            try
            {
                using (var ts = new TransactionScope())
                {
                    ...
                    for (int i = 0; i < data.Rows.Count; i++)
                    {
                        ...
                        var item = new LHSupplierPrice
                        {
                            FCode = data.Rows[i]["供应商编码"].ToString(),
                            FItemCode = data.Rows[i]["商品代码"].ToString(),
                            FPrice = Convert.ToDecimal(data.Rows[i]["采购单价"]),
                            FCompanyId = CurrentUser.AccountComId,
                            FFlag = 1
                        };

                        SupplierPriceService.AddEntity(item);
                    }

                    //写入数据库
                    SupplierPriceService.SaveChanges();
