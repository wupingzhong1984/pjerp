...
{
    public partial class InitStockQty : PageBase
    {
        /// <summary>
        ///     数据服务
        /// </summary>
        private InitStockService _initStockService;

        /// <summary>
        ///     数据服务
        /// </summary>
        protected InitStockService InitStockService
        {
            get { return _initStockService ?? (_initStockService = new InitStockService()); }
            set { _initStockService = value; }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Label1.Text = string.Format(@"引入本厂期初表");
            }
        }
            ...
            try
            {
                using (var ts = new TransactionScope())
                {
                    ...
                    for (int i = 0; i < data.Rows.Count; i++)
                    {
                        ...
                        var item = new LHInitStock
                        {
                            FCode = data.Rows[i]["商品代码"].ToString(),
                            FDate = data.Rows[i]["结算月份"].ToString(),
                            //FInitQty = Convert.ToDecimal(data.Rows[i]["数量"]),
                            //FInitKQty = Convert.ToInt32(data.Rows[i]["空瓶"]),
                            //FInitSQty = Convert.ToInt32(data.Rows[i]["实瓶"]),
                            FCompanyId = CurrentUser.AccountComId
                        };

                        InitStockService.AddEntity(item);
                    }

                    //写入数据库
                    InitStockService.SaveChanges();
